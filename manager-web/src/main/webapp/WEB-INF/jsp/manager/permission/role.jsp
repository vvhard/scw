...
<body>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<button type="button" class="btn btn-danger"
							style="float: right; margin-left: 10px;" onclick="deleteRoles()">
							<i class=" glyphicon glyphicon-remove"></i> 删除
						</button>
						<button type="button" class="btn btn-primary"
							style="float: right;"
							onclick="window.location.href='${ctp}/permission/role/add'">
	...
	<script type="text/javascript">
		...
		function pageQuery(pageno) {
			...
			$.ajax({
					type : "POST",
					url : "${ctp}/permission/role/pageQuery",
					data : jsonData,
					beforeSend : function() {
		...
		function goUpdatePage(id) {
			window.location.href = "${ctp}/permission/role/edit?id=" + id;
		}
		
		// 删除用户
		function deleteRole(id,name) {
			layer.confirm("是否删除角色【" + name + "】?", {
				icon : 3,
				title : '提示'
			}, function(cindex) {
				// 删除信息
				$.ajax({
					type : "POST",
					url : "${ctp}/permission/role/delete",
					data : {
						id : id
					},
					success : function(result) {
		...
		function deleteRoles() {
			...
			if (boxes.length == 0) {
				...
				layer.confirm("是否删除选中用户信息?", {
					...
					$.ajax({
						type : "POST",
						url : "${ctp}/permission/role/deletes",
		...
		function goAssignPage(id) {
			window.location.href = "${ctp}/permission/role/assign?id=" + id;
		}
	</script>
</body>
</html>
